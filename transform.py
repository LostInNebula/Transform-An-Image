...
def invert(img_matrix):
    ...
    for y in range(height):
        for x in range(width):
            ...
            img_matrix[y][x][0] = new_red
            img_matrix[y][x][1] = new_green
            img_matrix[y][x][2] = new_blue
    return img_matrix
...
def grayscale(img_matrix):
    ...
    for y in range(height):
        for x in range(width):
            ...
            old_green = img_matrix[y][x][1]
            old_blue = img_matrix[y][x][2]
            avg = int((old_red + old_green + old_blue)/3)
            for i in range(0,3):
                img_matrix[y][x][i] = avg
    return img_matrix
...
def mirror(img_matrix):
    ...
    height = len(img_matrix)
    width = len(img_matrix[0])
    bottom_half_length = int(height / 2)
    for x in range(width):
        for i in range(0, bottom_half_length):
            firsthalf_red = img_matrix[i][x][0]
            firsthalf_green = img_matrix[i][x][1]
            firsthalf_blue = img_matrix[i][x][2]
        for j in range(bottom_half_length, height):
            img_matrix[j] = img_matrix[(height-1) - j]
    return img_matrix
...
def right_diff(img_matrix):
    ...
    for y in range(height):
        for x in range(width - 1):
                for i in range(0, 3):
                    pixel = int(img_matrix[y][x][i])
                    next_pixel = int(img_matrix[y][x+1][i])

                    if ((pixel - next_pixel) * 8) > 255 :
                        new_pixel_val = 255
                    elif ((pixel - next_pixel) * 8) >= 0:
                        new_pixel_val = (pixel - next_pixel) * 8
                    else:
                        new_pixel_val = 0

                    img_matrix[y][x][i] = new_pixel_val
                    c += 1

        img_matrix[y][width - 1][0] = 0
        img_matrix[y][width - 1][1] = 0
        img_matrix[y][width - 1][2] = 0
    ...
    for y in range(height):
        for x in range(width):
            ...
            for j in range(-1,2):  #Loop through y-1, y, y+1
                for i in range(-1,2):  #Loop through x-1, x, next
